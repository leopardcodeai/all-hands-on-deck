...
struct DebugOverlayView: View {
            ...
            VStack(alignment: .leading, spacing: 4) {
                ...
                Button { withAnimation(.spring(response: 0.3)) { expanded.toggle() } } label: {
                    HStack(spacing: 8) {
                        ...
                        Text("FB:\(state.firebaseFramesWritten)/\(state.firebaseFramesRead) ")
                            .font(.system(size: 10, weight: .bold, design: .monospaced))
    ...
    private var detailPanel: some View {
        VStack(alignment: .leading, spacing: 3) {
            ...
            Divider().opacity(0.3)
            row("Firebase writes", "\(state.firebaseFramesWritten) ok / \(state.firebaseWritesFailed) fail")
            row("Firebase reads", "\(state.firebaseFramesRead) ok / \(state.firebaseReadsFailed) fail")
            row("Firebase msg polls", "\(state.firebaseMsgsPolled)")
